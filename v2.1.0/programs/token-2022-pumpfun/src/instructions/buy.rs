...
impl<'info> Buy<'info> {
    pub fn process(
        ...
        let estimated_out_token = ((sol_input_amt as f64).div((self.bonding_curve.init_virtual_sol + self.bonding_curve.sol_reserves) as f64).mul(self.bonding_curve.token_reserves as f64)) as u64;
        ...
        self.bonding_curve.sol_reserves += sol_input_amt;
        self.bonding_curve.token_reserves -= estimated_out_token;
        ...
        if self.bonding_curve.sol_reserves > self.global_configuration.bonding_curve_limitation {
            emit!(BondingCurveCompleted {
                ...
                userAta: self.user_ata.key(),
                solPool: self.sol_pool.key(),
                tokenPool: self.token_pool.key(),
            })
        }
